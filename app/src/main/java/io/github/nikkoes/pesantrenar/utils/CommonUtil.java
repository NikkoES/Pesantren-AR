...
//        Glide.with(context).load(Constans.WEB_URL_IMAGE + url).placeholder(circularProgressDrawable).into(imageView);
    ...
    public static String stringToDateFormat(String tanggal, String formatBefore, String formatTo) {
        SimpleDateFormat fromString = new SimpleDateFormat(formatBefore);
        Date date = null;
        try {
            date = fromString.parse(tanggal);
        } catch (ParseException e) {
        ...
        DateFormat dateFormat = new SimpleDateFormat(formatTo, Locale.ENGLISH);
        TimeZone tz = TimeZone.getTimeZone("Asia/Jakarta");
        dateFormat.setTimeZone(tz);
